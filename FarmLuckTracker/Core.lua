...
local function defaults(src, dst) dst = type(dst) == "table" and dst or {}; for k, v in pairs(src) do if type(v) == "table" then dst[k] = defaults(v, dst[k]) elseif dst[k] == nil then dst[k] = v end end; return dst end
...
function FLT:Color(luck)
    ...
    if luck and luck < 1 then local t = clamp(luck, 0, 1); r, g, b = 0.24 + (0.10 - 0.24) * t, 0.02 + (0.10 - 0.02) * t, 0.02 + (0.10 - 0.02) * t
    elseif luck and luck > 1 then local t = clamp((luck - 1) / 1.5, 0, 1); r, g, b = 0.10 + (0.95 - 0.10) * t, 0.10 + (0.62 - 0.10) * t, 0.10 + (0.05 - 0.10) * t end
...
function FLT:CreateUI()
    ...
    f:SetBackdropBorderColor(0.9, 0.72, 0.24, 0.85); local p = self.db.point; f:SetPoint(p[1], UIParent, p[2], p[3], p[4])
...
function FLT:Find(q) q = lower(trim(q)); local out = {}; if q == "" then for i = 1, math.min(5, #FARMS) do out[i] = FARMS[i] end; return out end; for _, f in ipairs(FARMS) do local h = lower((f.name or "") .. " " .. tostring(f.itemId or "")); for _, a in ipairs(f.aliases or {}) do h = h .. " " .. lower(a) end; if h:find(q, 1, true) then table.insert(out, f) end end; return out end
function FLT:RefreshSearch() local m = self:Find(self.search:GetText()); for i, row in ipairs(self.rows) do local f = m[i]; if f then row.key = f.key; row.text:SetText(f.name); row:Show() else row.key = nil; row:Hide() end end end
function FLT:Select(key) if BY_KEY[key] then self.db.selectedKey = key; self.search:SetText(BY_KEY[key].name); self.search:ClearFocus(); self:Refresh() end end

function FLT:RefreshSetup()
    local f = self:Farm(); if not f then return end; local a, b = self:Odds(f)
    self.title:SetText("FarmLuck"); self.lock:SetText(self.db.locked and "U" or "L"); self.selected:SetText(f.name .. "  |  " .. (f.sourceLabel or "Attempts")); self.targetLabel:SetText("Target"); self.target:SetText(tostring(self.db.targetAmount or 1)); self.odds1Label:SetText(f.mode == "twoStage" and "S1 %" or "Odds %"); self.odds1:SetText(string.format("%.3f", a * 100))
    ...
    self.locations:SetText(table.concat(f.locations or {}, "\n")); self.note:SetText((f.chanceLabel or f.stage1 and f.stage1.chanceLabel or "Odds") .. " - " .. (f.oddsNote or "Editable estimate.")); self:RefreshSearch(); self:Color(1)
end
...
function FLT:Stats()
    ...
    if f.mode == "twoStage" then st.a, st.b = s.stage1Chance or 1, s.stage2Chance or 1; st.overall = st.a * st.b; st.exp1, st.exp2, st.expAll = st.attempts * st.a, (s.stage2Attempts or 0) * st.b, st.attempts * st.overall; st.r1, st.r2, st.rAll = div(s.stage1Count or 0, st.exp1), div(st.acquired, st.exp2), div(st.acquired, st.expAll); local held = math.max(0, (s.stage1Count or 0) - (s.stage2Attempts or 0)); st.left = div(math.max(0, st.remaining - held * st.b), st.overall); st.luck = st.expAll >= 0.25 and st.rAll or st.exp1 >= 1 and st.r1 or 1 else st.a = s.chance or f.chance or 1; st.exp = st.attempts * st.a; st.obs = div(st.acquired, st.attempts); st.r = div(st.acquired, st.exp); st.left = div(st.remaining, st.a); st.luck = st.exp >= 0.25 and st.r or 1 end; return st
end

function FLT:RefreshSession()
    local st = self:Stats(); if not st then return end; local s, f = self.session, st.farm; self.title:SetText(f.name); self.lock:SetText(self.db.locked and "U" or "L"); self.addTry:SetText(f.attemptButton or "+ Try"); self.lines[1]:SetText("Goal " .. st.acquired .. "/" .. st.target .. " (" .. pct(st.acquired / st.target) .. ")"); self.lines[2]:SetText((f.sourceLabel or "Attempts") .. " " .. st.attempts .. "  |  Need about " .. num(st.left) .. " more")
    if f.mode == "twoStage" then self.lines[3]:SetText((f.stage1.label or "Stage 1") .. " " .. (s.stage1Count or 0) .. " / " .. num(st.exp1) .. " exp"); self.lines[4]:SetText((f.stage2.label or "Stage 2") .. " " .. st.acquired .. " / " .. num(st.exp2) .. " exp"); self.lines[5]:SetText("Opened " .. (s.stage2Attempts or 0) .. "  |  Overall " .. rate(div(st.acquired, st.attempts)) .. " per attempt"); self.lines[6]:SetText("Luck S1 " .. ratio(st.r1) .. "  |  S2 " .. ratio(st.r2) .. "  |  All " .. ratio(st.rAll)); self.lines[7]:SetText("Odds " .. pct(st.a) .. " then " .. pct(st.b)) else self.lines[3]:SetText((f.targetLabel or "Items") .. " " .. st.acquired .. " / " .. num(st.exp) .. " exp  |  " .. rate(st.obs) .. " per attempt"); self.lines[4]:SetText("Luck " .. ratio(st.r) .. "  |  Odds " .. pct(st.a)); self.lines[5]:SetText(f.oddsNote or ""); self.lines[6]:SetText("Manual " .. (s.manualAttempts or 0) .. " tries, " .. (s.manualItems or 0) .. " items"); self.lines[7]:SetText("") end
    self.lines[8]:SetText(""); self:Color(st.luck)
end

function FLT:Loot(msg) if not self.session then return end; local f, id = BY_KEY[self.session.farmKey], itemFromMsg(msg); if not f or not id then return end; local q = qtyFromMsg(msg); if f.mode == "twoStage" and f.stage1 and id == f.stage1.itemId then self:AddStage1(q) end; if id == f.itemId then self:AddItem(q, false) end end
...
function FLT:Slash(msg) local cmd, rest = trim(msg):match("^(%S*)%s*(.-)$"); cmd = lower(cmd); if cmd == "" or cmd == "toggle" then self.db.hidden = self.frame:IsShown(); show(self.frame, not self.db.hidden) elseif cmd == "show" then self.db.hidden = false; self.frame:Show() elseif cmd == "hide" then self.db.hidden = true; self.frame:Hide() elseif cmd == "lock" then self.db.locked = true; self:Refresh() elseif cmd == "unlock" then self.db.locked = false; self:Refresh() elseif cmd == "scale" then local s = tonumber(rest); if s then self.db.scale = clamp(s, 0.5, 2.5); self.frame:SetScale(self.db.scale) end elseif cmd == "end" then self:End() else DEFAULT_CHAT_FRAME:AddMessage("|cffffcc33FarmLuck|r /flt show, hide, lock, unlock, scale <n>, end") end end

function FLT:Loaded(name)
    if name ~= addonName then return end; FarmLuckTrackerDB = defaults(DEFAULT_DB, FarmLuckTrackerDB); self.db = FarmLuckTrackerDB; if self.db.session and BY_KEY[self.db.session.farmKey] then self.session = self.db.session end; self:CreateUI(); SLASH_FARMLUCKTRACKER1 = "/flt"; SLASH_FARMLUCKTRACKER2 = "/farmluck"; SlashCmdList.FARMLUCKTRACKER = function(msg) self:Slash(msg) end; if C_Container and C_Container.UseContainerItem then hooksecurefunc(C_Container, "UseContainerItem", function(bag, slot) self:ContainerUse(bag, slot) end) end; if UseContainerItem then hooksecurefunc("UseContainerItem", function(bag, slot) self:ContainerUse(bag, slot) end) end
end

FLT:SetScript("OnEvent", function(self, event, ...)
    if event == "ADDON_LOADED" then self:Loaded(...)
    elseif event == "CHAT_MSG_LOOT" then self:Loot(...)
    elseif event == "COMBAT_LOG_EVENT_UNFILTERED" then local _, subevent; if CombatLogGetCurrentEventInfo then _, subevent = CombatLogGetCurrentEventInfo() else _, subevent = ... end; local f = self.session and BY_KEY[self.session.farmKey]; if subevent == "PARTY_KILL" and f and f.sourceKind == "kill" then self:AddAttempt(1, false) end
